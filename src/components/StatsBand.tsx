...
const arStats = [
  { value: "120+", label: "مشروعاً منجزاً" },
  { value: "48h", label: "للرد الأول" },
  { value: "4.9/5", label: "رضا العملاء" },
  { value: "المغرب + دولياً", label: "نطاق العمل" },
];
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-10">
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-3xl border border-white/10 bg-white/[0.015] backdrop-blur-md md:grid-cols-4">
          {items.map((s) => (
            <div
              key={s.label}
              className="relative bg-black/45 px-5 py-7 text-center md:px-8 md:py-12 md:text-left rtl-text-right"
            >
              <div
                aria-hidden
                className="absolute left-5 top-5 h-6 w-px bg-gradient-to-b from-cyan-200/60 to-transparent md:left-8 md:top-8 md:h-7"
              />
              <div className="cinematic-text text-3xl font-light tracking-tight text-white md:text-5xl">
                {s.value}
              </div>
              <div className="mt-3 text-[10px] uppercase tracking-[0.28em] text-white/52 md:mt-4 md:text-[11px]">
                {s.label}
              </div>
            </div>
          ))}
