...
function PortfolioCard({ label, category, result, tag, config, index, projectLink, href }: CardProps) {
  return (
    <motion.article
      ...
      className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_18px_54px_-36px_rgba(15,23,42,0.55)] transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-[0_24px_70px_-38px_rgba(15,23,42,0.68)]"
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h3 className="mt-4 text-xl font-light leading-tight tracking-tight text-slate-950">
          {label}
        </h3>
        <p className="mt-3 text-sm leading-6 text-slate-600">{tag}</p>
...
export default function CreativeFormats({ locale }: { locale?: Locale }) {
  ...
  return (
    <section className="relative overflow-hidden bg-white px-4 pb-14 pt-10 sm:px-6 md:pb-20 md:pt-16 lg:px-10">
      <div className="relative mx-auto max-w-7xl">
        {/* Header */}
        <div className="mb-8 flex min-w-0 flex-col gap-5 md:mb-10 md:flex-row md:items-end md:justify-between rtl-md-row">
          <SectionLabel
            kicker={t.kicker}
            title={t.title}
            subtitle={t.subtitle}
        ...
        {featuredFormat && (
          <motion.article
            ...
            className="group overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_28px_90px_-52px_rgba(15,23,42,0.72)] transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-[0_34px_110px_-54px_rgba(15,23,42,0.82)] lg:grid lg:grid-cols-[1.35fr_0.85fr]"
          >
            <div className="min-h-[320px] overflow-hidden bg-slate-100 sm:min-h-[420px] lg:min-h-[520px]">
              <img
            ...
            <div className="flex min-w-0 flex-col justify-start p-6 sm:p-8 lg:p-10">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                ...
                <h3 className="mt-6 max-w-xl text-3xl font-light leading-[1.05] tracking-tight text-slate-950 sm:text-4xl lg:text-5xl">
              ...
              <Link
                href={projectHref}
                className="mt-10 inline-flex w-fit items-center gap-2 rounded-full border border-slate-200 bg-slate-950 px-5 py-3 text-sm font-medium text-[#f8fafc] transition duration-300 hover:-translate-y-0.5 hover:gap-3 hover:bg-blue-700"
              >
                {t.projectLink}
                <span aria-hidden className="rtl-arrow">→</span>
              </Link>
        ...
        <motion.div layout className="mt-6 grid grid-cols-1 items-stretch gap-5 md:grid-cols-2 lg:grid-cols-3">
