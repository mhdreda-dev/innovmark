  ...
  return (
    <div className={`reveal-on-scroll flex min-w-0 flex-col gap-3 md:gap-4 ${alignment}`}>
      <div className="flex max-w-full items-center gap-2 sm:gap-3">
        <span className="h-px w-6 shrink-0 bg-gradient-to-r from-cyan-200 via-blue-300 to-violet-400 sm:w-8" />
        <span className="cinematic-text text-[10px] uppercase tracking-[0.14em] text-blue-700/72 md:text-[11px] md:tracking-[0.46em]">
          {kicker}
        </span>
      </div>
      <h2 className="cinematic-text max-w-[780px] text-[1.65rem] font-light leading-[1.12] tracking-tight text-slate-950 sm:text-3xl md:text-5xl md:leading-[1.08]">
        {title}
      </h2>
      {subtitle && (
